...
RSpec.describe 'IIIF API' do
  ...
  context 'with versioned file layout' do
    ...
    describe 'metadata requests' do
      ...
      context 'where no one can download' do
        let(:public_json) do
          Factories.legacy_cocina_with_file(file_access: { 'view' => 'world', 'download' => 'none' })
        end
      ...
      context 'where stanford only no download rights' do
        let(:public_json) do
          Factories.legacy_cocina_with_file(file_access: { 'view' => 'stanford', 'download' => 'none' })
        end
  ...
  context 'with legacy file layout' do
    let(:public_json) do
    ...
    describe 'metadata requests' do
      ...
      context 'for stanford-restricted documents' do
        ...
        it 'redirects requests to the degraded info.json' do
          get '/image/iiif/nr349ct7889/image/info.json'
          expect(response).to have_http_status :redirect
          expect(response).to redirect_to('/image/iiif/degraded/nr349ct7889/image/info.json')
          expect(response.headers['Cache-Control']).to match(/max-age=0/)
        end
