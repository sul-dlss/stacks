...
RSpec.describe StacksMediaToken do
  ...
  describe '#create_from_encrypted_string' do
    it 'should build an object with the right fields' do
      ...
      token_from_encrypted_str = described_class.send(:create_from_encrypted_string, encrypted_token_str)
  ...
  describe '#token_valid?' do
    ...
    it 'returns false if the token is too old' do
      expired_timestamp = (described_class.max_token_age + 2.seconds).ago
  ...
  describe '#verify_encrypted_token?' do
    ...
    it 'returns true for a valid token that matches the specified values' do
      expect(described_class.verify_encrypted_token?(enc_token_str, id, file_name, user_ip)).to be true
    end

    it 'returns false if the signature is invalid' do
      invalid_sig_err = ActiveSupport::MessageVerifier::InvalidSignature
      allow(described_class).to receive(:create_from_encrypted_string).with(enc_token_str).and_raise(invalid_sig_err)
      expect(described_class.verify_encrypted_token?(enc_token_str, id, file_name, user_ip)).to be false
    end

    it 'returns false if the encrypted message is invalid' do
      invalid_msg_err = ActiveSupport::MessageEncryptor::InvalidMessage
      allow(described_class).to receive(:create_from_encrypted_string).with(enc_token_str).and_raise(invalid_msg_err)
      expect(described_class.verify_encrypted_token?(enc_token_str, id, file_name, user_ip)).to be false
    end

    it 'returns false on a token that has been tampered with' do
      invalid_token = "#{enc_token_str}invalid"
      expect(described_class.verify_encrypted_token?(invalid_token, id, file_name, user_ip)).to be false
    end

    it 'returns false if token_valid? returns false' do
      mock_token = double(described_class)
      allow(described_class).to receive(:create_from_encrypted_string).with(enc_token_str).and_return(mock_token)
  ...
  describe 'field validation' do
    ...
    it 'can create a valid token without raising an error' do
      expect { described_class.new(valid_id1, valid_filename, valid_ip) }.not_to raise_error
      expect { described_class.new(valid_id2, valid_filename, valid_ip) }.not_to raise_error
    end

    it 'raises an error when creating a token with an empty required field' do
      expect { described_class.new('', valid_filename, valid_ip) }
        .to raise_error(ActiveModel::StrictValidationFailed, "Id can't be blank")
      expect { described_class.new(valid_id1, '', valid_ip) }
        .to raise_error(ActiveModel::StrictValidationFailed, "File name can't be blank")
      expect { described_class.new(valid_id1, valid_filename, '') }
    ...
    it 'raises an error when creating a token with a bad id' do
      expect { described_class.new('zab012cd34567', valid_filename, valid_ip) }
        .to raise_error(ActiveModel::StrictValidationFailed, 'Id is invalid')
    end

    it 'raises an error when creating a token with a bad IP' do
      expect { described_class.new(valid_id1, valid_filename, '127') }
        .to raise_error(ActiveModel::StrictValidationFailed, 'User ip is invalid')
      expect { described_class.new(valid_id1, valid_filename, '0') }
        .to raise_error(ActiveModel::StrictValidationFailed, 'User ip is invalid')
      expect { described_class.new(valid_id1, valid_filename, '0.0.0.0.0') }
        .to raise_error(ActiveModel::StrictValidationFailed, 'User ip is invalid')
      expect { described_class.new(valid_id1, valid_filename, 'localhost') }
        .to raise_error(ActiveModel::StrictValidationFailed, 'User ip is invalid')
    end
  end
end
