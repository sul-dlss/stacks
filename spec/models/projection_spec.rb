...
RSpec.describe Projection do
  ...
  describe '#thumbnail?' do
    ...
    context "when equal to 400 on both edges" do
      let(:transformation) { IiifTransformation.new(size: '400,400', region: 'full') }

      it { is_expected.to be true }
    end

    context "when equal to 400 on height" do
      let(:transformation) { IiifTransformation.new(size: '400,', region: 'full') }

      it { is_expected.to be true }
    end

    context "when bigger than 400 on one edges" do
      let(:transformation) { IiifTransformation.new(size: '401,400', region: 'full') }

      it { is_expected.to be false }
    end

    context "when bigger than to 400 on height" do
      let(:transformation) { IiifTransformation.new(size: '401,', region: 'full') }
    ...
    context "when the transformation doesn't select the full region" do
      let(:transformation) { IiifTransformation.new(region: '0,0,1,1', size: 'full') }
  ...
  describe '#tile?' do
    let(:transformation) { IiifTransformation.new(size: 'full', region: 'full') }
    subject { instance.tile? }

    context 'when an absolute region is specified' do
      let(:transformation) { IiifTransformation.new(size: 'full', region: '0,0,1,1') }
      it { is_expected.to be true }
    end
    ...
    context "when it is smaller than 512px on long edge scaled width & height" do
      let(:transformation) { IiifTransformation.new(size: '256,256', region: '0,0,256,256') }
      it { is_expected.to be true }
    end

    context "when it is smaller than 512px on long edge scaled height" do
      let(:transformation) { IiifTransformation.new(size: '256,', region: '0,0,256,256') }
      it { is_expected.to be true }
    end
    ...
    context "when it is larger than 512px on long edge scaled width & height" do
      let(:transformation) { IiifTransformation.new(size: '513,513', region: '0,0,1,1') }
      it { is_expected.to be false }
    end

    context "when it is larger than 512px on long edge scaled width" do
      let(:transformation) { IiifTransformation.new(size: ',513', region: '0,0,800,600') }
      it { is_expected.to be false }
    end
  end
  ...
  describe '#region_dimensions' do
    ...
    context 'for a percentage scale down' do
      let(:transformation) { IiifTransformation.new(size: 'full', region: 'pct:50') }
    ...
    context 'for a percentage scale up' do
      let(:transformation) { IiifTransformation.new(size: 'full', region: 'pct:200') }
    ...
    context 'for an explicit region' do
      let(:transformation) { IiifTransformation.new(size: 'full', region: '0,1,2,3') }
