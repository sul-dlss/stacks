  ...
  describe 'iiif image api requests' do
    ...
    context '#show: identifier with' do
      ...
      it 'some chars requiring URI escaping' do
        ...
        expect(get: "/image/iiif/#{ERB::Util.url_encode(identifier)}/info.json").to route_to('iiif#metadata', identifier: identifier)
      end

      it 'square brackets must be url escaped' do
        identifier = 'foo[brackets].bar.pdf'
        escaped_identifier = ERB::Util.url_encode identifier
      ...
      it 'ü must be url escaped' do
        ...
        identifier = "fü.pdf"
        escaped_identifier = ERB::Util.url_encode(identifier) # becomes %C3%BC
      ...
      it '%20 in actual name (from ck155rf0207)' do
        ...
        expect(get: "/image/iiif/#{ERB::Util.url_encode(identifier)}").to route_to('iiif#show', identifier: identifier)
      end
    end
  end
